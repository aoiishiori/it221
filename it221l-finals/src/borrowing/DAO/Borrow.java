 ...
 *
 * Fixes applied:
 *   1. countActiveBorrows() — replaced incorrect CALL syntax for a MySQL stored
 *      function with a PreparedStatement SELECT call (MySQL functions are not
 *      invoked via CALL; only stored procedures are).
 *   2. createBorrowTransaction() — added AVAILABLE-status check before insert
 *      to prevent double-borrowing.
...
public class Borrow implements BorrowInter {
    ...
    // ----------------------------------------------------------------
    // CREATE — borrow transaction (with availability check)
    // ----------------------------------------------------------------
    @Override
    public boolean createBorrowTransaction(String borrowerId, String custodianId,
        ...
        try {
            ...
            // FIX: check every barcode is AVAILABLE before inserting
            String checkSql = "SELECT barcode, status FROM equipment " +
    ...
    // ----------------------------------------------------------------
    // CallableStatement — stored FUNCTION  (FIX: use SELECT, not CALL)
    // MySQL stored functions are called with SELECT fn_name(arg),
    // not with the CALL keyword (which is for stored procedures only).
    // ----------------------------------------------------------------
    @Override
    ...
    // ----------------------------------------------------------------
    // CallableStatement — stored PROCEDURE (correct: uses CALL)
    // ----------------------------------------------------------------
    @Override
