...
public class Borrow implements BorrowInter {
    ...
    public List<DataClass.BorrowRecord> getAllRecords() {
        String sql = "SELECT bt.transaction_id, CONCAT(p.last_name, ', ', p.first_name, ' (', IFNULL(ua.username, 'no-user'), ')') AS borrower_name, bt.borrower_id, bt.class_id, ar.activity_name, DATE_FORMAT(bt.borrow_date, '%Y-%m-%d %H:%i') AS borrow_date, bt.expected_return, DATE_FORMAT(bt.return_date, '%Y-%m-%d %H:%i') AS return_date, bt.transaction_status FROM BORROW_TRANSACTION bt JOIN PERSON p ON bt.borrower_id = p.person_id LEFT JOIN USER_ACCOUNT ua ON p.person_id = ua.person_id LEFT JOIN ACTIVITY_REQUEST ar ON bt.request_id = ar.request_id ORDER BY bt.borrow_date DESC";
